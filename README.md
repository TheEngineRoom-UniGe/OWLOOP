...
>Nevertheless, this API implements an efficient, customizable and general (but jet simple) synchronization interface that allows developers to easier design more flexible systems. While OWLOOP cares at run time to map the reasoned ontological representation in their objects. 
> 
...
OWLOOP as been developed on top of the [aMOR](https://github.com/EmaroLab/multi_ontology_reference), framework which is a collection of helpers methods based on [OWL API](https://github.com/owlcs/owlapi). Those are still completely accessible through this API and allow to work with many reasoners, we test the software with [Pellet](https://github.com/Galigator/openllet). 
...
OWLOOP is organized (in the [src](https://github.com/EmaroLab/owloop/tree/master/src) folder) as follow:
 ...
 - `test/` : contains the code used to check for bugs within a testing driven implementation style. Beside, even if those implementations does not do meaningful semantic manipulations, those are good examples to get started with features of this API.
  - `main/java/it/emarolab/owloop/` : contains the actual library components, dived in:
    - `core/` : contains the interfaces of the synchronization procedures, which has been generalized and it does not depend on OWL API. Those are:
      - `Semantic` : defines all the basic OWLOOP interfaces (i.e: `Ground`, `Axioms` and `Descriptor`).
      - `Concept`, `Individual`, `Data` and `Object` properties : define the particular *Semantics* for different ontological entities.
...
More in details, the main ability of a *Descriptor* (or combination of those) are:
 - **read** : it modify its state in order to be equal to the described semantics in the ontological representation. It also returns that changes of such a manipulation. Note, that this does not automatically call the reasoning task but check in the reasoner state (it can be disabled).
...
Noteworthy, each `Descriptor` has methods to interact with the instantiated ontology through standard [aMOR](https://github.com/EmaroLab/multi_ontology_reference) helper functions as well as lower layer [OWL API](https://github.com/owlcs/owlapi). From here you can not only call the reasoning task but also get the `OWLManager`, `OWLDataFactory`, `OWLOntology` and `OWLReferences`. So, other manipulation and queries are possible without further dependences or bridges.

#### API Capability

Follow a list of "base" implemented *Descriptor* interfaces, divided for the type of *Ground* that they can manipulate. So, within the same table, they can be aggregated in any combinations. Note that the *Restriction* types of *Axioms* cannot be *build*.
...
| ObjectLink | OWLObjectProperty set of OWLNamedIndividual |    the specified object properties values applied to the ground |
|   DataLink | OWLDataProperty set of OWLLiteral           |    the specified data properties values belonging to the ground |
...
| Equivalent | OWLObjectProperty |                                          all the equivalent data properties to the ground |
|        Sub | OWLObjectProperty |                                                 all the sub data properties of the ground |
|      Super | OWLObjectProperty |                                               all the super data properties of the ground |
